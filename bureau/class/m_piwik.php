    ...
    function user_add($user_login, $user_mail) {
        ...
        if ($api_data) {
            if ($api_data->result === 'success') {
                ...
                $ret_value = $db->query("INSERT INTO piwik_users (uid, passwd, login, created_date, email) VALUES ( ?, ?, ?, ?, ?);", array($cuid, md5($user_pass), $user_login, $user_creation_date, $user_mail));
                return $ret_value;
            } else {
